        ...
        <div className="fixed inset-0 z-[200] flex animate-fadeIn">
          <div className="w-72 bg-[#1a1a1a] border-r border-[#E2725B]/30 p-8 shadow-2xl flex flex-col justify-between">
            <nav className="space-y-8 mt-10">
              ...
              <button onClick={() => {setView('pairings'); setMenuOpen(false);}} className="block text-2xl font-serif hover:text-[#E2725B]">🍖 Food Match</button>
            </nav>
          ...
          <div className="animate-fadeIn">
            ...
            {REGIONS_DATA.map(country => (
              <div key={country.id} onClick={() => {setActiveItem(country); setView('country');}} 
                   className="p-8 bg-[#1c1c1c] rounded-[2rem] border border-white/5 mb-6 hover:border-[#E2725B]/40 cursor-pointer shadow-lg active:scale-95 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {activeItem.subRegions.map((reg, idx) => (
                <div key={idx} onClick={() => {setActiveItem(reg); setView('region');}} 
                  ...
                  <p className="text-[10px] text-[#E2725B] uppercase font-black mt-2">View {reg.wines.length} Iconic Wines</p>
                </div>
              ))}
            </div>
          </div>
            ...
            {activeItem.wines.map((wine, i) => (
              <div key={i} className="mb-12 border-b border-white/5 pb-10">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-[#E2725B] text-xs font-mono px-3 py-1 bg-[#E2725B]/10 rounded-full border border-[#E2725B]/20">{wine.grape}</span>
                </div>
